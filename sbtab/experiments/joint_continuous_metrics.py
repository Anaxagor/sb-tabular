...
from sbtab.solvers.continuous_time.joint_distribution.boosting.ipf_dsb.solver import JointContinuousBoostedSolver, JointContinuousBoostedConfig
...
def build_config(best: Dict, seed: int) -> JointContinuousBoostedConfig:
    cat_cfg = CatBoostContinuousJointConfig(
    ...
    return JointContinuousBoostedConfig(
...
def main() -> None:
    ...
    ap.add_argument("--outdir", type=str, default="joint_continuous_kfold_eval")
    ...
    for ds_name in ds_list:
        ...
        print("\n" + "=" * 100)
        print(f"JOINT CONTINUOUS BOOSTED | DATASET: {ds_name}")
        print("=" * 100)
        ...
        for fold_id, (train_idx, test_idx) in enumerate(kf.split(np.arange(len(df)))):
            ...
            model = JointContinuousBoostedSolver(dim=len(cols), cfg=cfg)
            model.fit(train_df)

            x_synth = model.sample(n=len(test_df))
            synth_df = pd.DataFrame(x_synth, columns=cols)
        ...
        summary = {
            ...
            "solver_config": {
                ...
                "alpha_ou": cfg.alpha_ou, "seed": cfg.seed,
            },
        }
