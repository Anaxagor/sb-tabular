...
def make_objective_for_dataset(
    ...
    """
    Objective function factory for CTGAN tuning.
    Uses fixed train/test split and fixed preprocessing.
    """
    def objective(trial: optuna.Trial) -> float:
        # --- hyperparameter search space (CTGAN spec) ---
        ...
        lr = trial.suggest_float("generator_lr", 1e-4, 5e-4, log=True)

        cfg = CTGANConfig(
            epochs=int(epochs),
            batch_size=int(batch_size),
            embedding_dim=int(embedding_dim),
            generator_dim=(gen_disc_width, gen_disc_width),
            discriminator_dim=(gen_disc_width, gen_disc_width),
            generator_lr=float(lr),
            discriminator_lr=float(lr),
            pac=int(pac),
            cuda=(device == "cuda"),
            seed=seed,
            verbose=False,
...
def main() -> None:
    ...
    ap.add_argument("--outdir", type=str, default="ctgan_optuna_results", help="Folder for CSV summaries")
