...
"""
K-fold evaluation for TabDDPM under the new mixed-type repository logic.
...
Main updates vs the old script:
  ...
  - uses TabularDataModule.prepare_kfold/get_fold instead of manual sklearn KFold,
    so preprocessing is fitted on each train fold only
  - passes both schema and fitted split-specific transforms into TabDDPMWrapper.fit(...)
  - evaluates on the processed fold representation returned by the DataModule

Assumptions:
  - the updated TabDDPMWrapper is used (the one that accepts schema + transforms and
    reconstructs numerical/categorical blocks from the preprocessed DataFrame).
  - target columns listed below are numeric-like for utility evaluation with R².
"""
...
# ---------------------------------------------------------------------
# Metrics in processed space
# ---------------------------------------------------------------------

def _common_numeric_cols(real: pd.DataFrame, synth: pd.DataFrame) -> List[str]:
...
# ---------------------------------------------------------------------
# Config / schema / transforms helpers
# ---------------------------------------------------------------------
...
def build_tabddpm_config_from_best(best: Dict, seed: int, device: str) -> TabDDPMConfig:
    """
    Support both the new tuning script keys and older JSONs.
    """
    n_epochs = int(best.get("n_epochs", best.get("n_iter", 1000)))
    num_timesteps = int(best.get("num_timesteps", 1000))
    batch_size = int(best.get("batch_size", 4096))
    lr = float(best.get("lr", 1e-3))

    n_layers = int(best.get("n_layers", 4))
    layer_size = int(best.get("layer_size", 256))
    d_layers = list(best.get("d_layers", [layer_size] * n_layers))
...
def main() -> None:
    ...
    ap.add_argument("--best_json_dir", type=str, default="sbtab/experiments/tuning_script/tabddpm_optuna_results/")
    ap.add_argument("--outdir", type=str, default="tabddpm_kfold_eval")
    ...
    for ds_name in ds_list:
        ...
        print("\n" + "=" * 100)
        print(f"TABDDPM EVALUATION: {ds_name}")
        print("=" * 100)

        df = my_data[ds_name].copy()
        target_col = resolve_target_col(df, ds_name)

        # Infer schema from raw dataframe, with target excluded from feature groups
        ...
        # Fixed KFold under the new module logic
        ...
        cfg = build_tabddpm_config_from_best(best_params, args.seed, args.device)
        ...
        for fold_id in range(args.n_splits):
            ...
            # Train TabDDPM on the processed train fold
            ...
            # Metrics in processed space
            metric_cols = _common_numeric_cols(test_proc, synth_proc)
            ...
            # Utility in processed space:
            # use all processed columns except target/id as features
            exclude = {c for c in [schema.target_col, schema.id_col] if c is not None}
            feature_cols_proc = [c for c in train_proc.columns if c not in exclude]
