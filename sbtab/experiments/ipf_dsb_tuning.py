...
def make_objective_for_dataset(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        num_steps = trial.suggest_int("num_steps", 16, 64, step=8)
        alpha_ou = trial.suggest_float("alpha_ou", 0.5, 1.5, step=0.1)
        ...
        ipf_iters = trial.suggest_int("ipf_iters", 5, 15, step=1)
        steps_per_phase = trial.suggest_int("steps_per_phase", 500, 2000, step=100)
        batch_size = trial.suggest_categorical("batch_size", [1024, 2048, 4096])

        # steps_per_phase = epochs * cache_batches; fix cache_batches for stable tuning
        cache_batches = 200
