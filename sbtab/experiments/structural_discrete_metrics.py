...
def build_structural_discrete_config(best: Dict, seed: int) -> StructuralDiscreteBoostedConfig:
    cat_cfg = CatBoostDiscreteScalarConfig(
    ...
    return StructuralDiscreteBoostedConfig(
        num_steps=int(best.get("num_steps", 20)),
        ipf_iters=int(best.get("ipf_iters", 3)),
...
def main() -> None:
    ...
    ap.add_argument("--outdir", type=str, default="structural_discrete_kfold_eval")
    ...
    for ds_name in ds_list:
        ...
        print("\n" + "=" * 100)
        print(f"STRUCTURAL DISCRETE BOOSTED | DATASET: {ds_name}")
        print("=" * 100)
        ...
        cfg = build_structural_discrete_config(best_params, seed=args.seed)
        ...
        for fold_id, (train_idx, test_idx) in enumerate(kf.split(np.arange(len(df)))):
            ...
            model = StructuralDiscreteBoostedSolver(cfg=cfg)
            model.fit(train_df)
