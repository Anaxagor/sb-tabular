...
"""
Example: training and sampling with LightSB in sb-tabular
=========================================================

This example assumes:
  - continuous tabular data
  - preprocessing is done upstream (DropMissingRows + StandardScaler)
  - LightSB is used as a generative solver on the preprocessed train split

You can adapt this example to any DataFrame with only continuous columns.
...
def main() -> None:
    ...
    model_cfg = LightSBPotentialConfig(
    n_potentials = 50,
    epsilon = 0.1,
    is_diagonal = True,
    ...
    # 4) Train LightSB
    # ---------------------------------------------------------
    solver = LightSBSolver(dim=dim, cfg=solver_cfg)
    solver.fit(train_df)
    ...
    x_synth = solver.sample(n=n_synth, seed=123)
